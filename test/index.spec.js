...
describe("loadConfig", async () => {
	it.only("should return config", async () => {
		const configFilePath = pathModule.resolve(__dirname, "configs", "b.cjson");
		const config = await loadConfig(configFilePath);
		/* remove-console */ /* prettier-ignore */ console.log(`>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Line: 10, File: index.spec.js ~ printing(config) <<<<<<<<<<<<<<<<<<<<<<<<<< \n  ${JSON.stringify(config, null, 2)}`);
		assert.equal(config.table.dimensions.width, "120 cm");
	});
